...
try {
  if (fs.statSync(LOG_FILE).size > 10 * 1024 * 1024)
    fs.renameSync(LOG_FILE, LOG_FILE + ".old");
} catch (e) {}
const logStream = fs.createWriteStream(LOG_FILE, { flags: "a" });

function ts() {
  return new Date().toLocaleString("sv", { timeZone: "Europe/Berlin" });
}
...
const VIDEO_AMOUNT = 32; // update this when adding/removing videos
...
// defines which screens each screen can drift to next
const SCREEN_ROUTES = {
  2: ["6"],
  3: ["4", "5", "2", "2", "2"],
  4: ["3", "5", "2", "2"],
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("close", () => {
    ...
    console.log(
      `Device disconnected: ${deviceId} — connected: ${[...clients.keys()].join(", ") || "none"}`,
    );
    if (wasActive) {
      nowPlaying = null;
      if (driftMode) driftStep(deviceId);
...
function switchToClient(deviceId) {
  ...
  client.send(
    JSON.stringify({ action: "play", file: scenes[sceneId].video, timestamp }),
  );
  nowPlaying.deviceId = deviceId;
...
function pickNextDevice(excludeDeviceId = null) {
  const routes = excludeDeviceId ? SCREEN_ROUTES[excludeDeviceId] : null;
  const connected = [...clients.keys()];
  const candidates = routes
    ? connected.filter((id) => routes.includes(id))
    : connected.filter((id) => id !== excludeDeviceId);
...
function driftStep(excludeDeviceId = null) {
  if (!driftMode) return;
  if (clients.size === 0)
    return console.log("Drift: no devices connected, waiting...");
  ...
  if (driftSceneId === null) {
    ...
    driftSceneId = ((parseInt(driftSceneId, 10) % VIDEO_AMOUNT) + 1)
      .toString()
      .padStart(2, "0");
  }

  const deviceId = pickNextDevice(excludeDeviceId);
  ...
  if (driftTrace && excludeDeviceId && previousSceneId) {
    ...
    if (traceClient) {
      ...
      traceClient.send(
        JSON.stringify({ action: "play", file: traceFile, timestamp: 0 }),
      );
...
console.log(
  "Commands: play <id> on <device>  |  switch <device>  |  stop [device]  |  drift [--trace]  |  party  |  scatter  |  testpattern  |  restart  |  quit",
);
...
rl.on("line", (input) => {
  ...
  if (cmd === "play" && parts[2] === "on" && parts[3])
    return startVideo(parts[3], parts[1]);
  ...
  if (cmd === "testpattern") {
    ...
    clients.forEach((ws, deviceId) => {
      ws.send(
        JSON.stringify({ action: "play", file: "test.png", timestamp: 0 }),
      );
  ...
  if (cmd === "scatter") {
    nowPlaying = null;
    clients.forEach((ws, deviceId) => {
      const sceneId = (Math.floor(Math.random() * VIDEO_AMOUNT) + 1)
        .toString()
        .padStart(2, "0");
      ws.send(
        JSON.stringify({
          action: "play",
          file: scenes[sceneId].video,
          timestamp: 0,
        }),
      );
  ...
  if (cmd === "party") {
    ...
    partyInterval = setInterval(() => {
      ...
      const deviceId = pickNextDevice(lastDeviceId);
